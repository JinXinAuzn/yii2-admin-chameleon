...
namespace jx\admin_chameleon\models;
...
{
	const CACHE_TAG = 'jx.admin_chameleon.route';
	...
	public function addNew($routes)
	{
		...
		foreach ($routes as $route) {
			try {
				...
				$item->description = $data[1] ? $data[1] : $data[0];
				if (count($r) > 1) {
					$action = '/' . trim($r[0], '/');
	...
	public function getRoutes()
	{
		...
		foreach ($manager->getPermissions() as $name) {
			...
			$exists[] = $name->description ? $name->name . '|' . $name->description : $name->name;
			unset($routes[$name->description ? $name->name . '|' . $name->description : $name->name]);
		}
		return [
	...
	protected function getRouteRecursive($module, &$result)
	{
		...
		try {
			...
			$all = '/' . ltrim($module->uniqueId . '/*', '/') . '|' . Yii::t('rbac-admin', '/' . ltrim($module->uniqueId . '/*', '/'));
			$result[$all] = $all;
		} catch (\Exception $exc) {
			Yii::error($exc->getMessage(), __METHOD__);
		}
	...
	protected function getControllerActions($type, $id, $module, &$result)
	{
		...
		try {
			...
			$all = "/{$controller->uniqueId}/*|" . Yii::t('rbac-admin', "/{$controller->uniqueId}/*");
			$result[$all] = $all;
		} catch (\Exception $exc) {
			Yii::error($exc->getMessage(), __METHOD__);
		}
	...
	protected function getActionRoutes($controller, &$result)
	{
		...
		try {
			...
			foreach ($controller->actions() as $id => $value) {
				$result[$prefix . $id . '|' . Yii::t('rbac-admin', $prefix . $id)] = $prefix . $id;
			}
			$class = new \ReflectionClass($controller);
			foreach ($class->getMethods() as $method) {
				$name = $method->getName();
				$descriptions = $method->getDocComment();
				$description = [];
				if ($method->isPublic() && !$method->isStatic() && strpos($name, 'action') === 0 && $name !== 'actions'/* && strpos($description, '@description')!== false*/) {
					preg_match('/\@description [\x{4e00}-\x{9fa5}]+/u', $descriptions, $description);
					$des = mb_substr(current($description), 13, null, 'UTF-8');
					$name = strtolower(preg_replace('/(?<![A-Z])[A-Z]/', ' \0', substr($name, 6)));
					$id = $prefix . ltrim(str_replace(' ', '-', $name), '-');
					$v=empty($des)?$id.'|'.$id:$id.'|'.$des;
					$result[$v] = $v;
				}
			}
		} catch (\Exception $exc) {
