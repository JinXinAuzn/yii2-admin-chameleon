...
class AuthItem extends Model
{
	...
	public function getItems()
	{
		...
		foreach ($manager->getPermissions() as $name) {
			$available[$name->description ? $name->name . '|' . $name->description : $name->name] = $name->name[0] == '/' ? 'route' : 'permission';
		}
		$assigned = [];
		foreach ($manager->getChildren($this->_item->name) as $item) {
			$assigned[$item->description ? $item->name . '|' . $item->description : $item->name] = $item->type == 1 ? 'role' : ($item->name[0] == '/' ? 'route' : 'permission');
			unset($available[$item->description?$item->name.'|'.$item->description:$item->name.'|'.$item->name]);
		}

		unset($available[$this->name.'|'.$this->description]);
